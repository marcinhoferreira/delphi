...
type
  TModelEntidadePedido = class(TDataModule)
    ...
    procedure qryPedidosBeforeClose(DataSet: TDataSet);
  private
  ...
  public
    ...
    function GetPedidoNumero: Integer;
    ...
    procedure InserePedido(APedido: TWKPedido);
    procedure DeletaItem(AItem: TWKItemPedido);
  end;
...
procedure TModelEntidadePedido.DeletaItem(AItem: TWKItemPedido);
var
   AQuery: TFDQuery;
begin
   ...
   try
      ...
      with AQuery.SQL do
         begin
            Clear;
            Add('DELETE FROM pedidos_produtos');
            Add('WHERE id = :id');
         end;
      // Definição dos parâmetros
      AQuery.ParamByName('id').DataType := ftInteger;
      AQuery.ParamByName('id').ParamType := ptInput;
      AQuery.ParamByName('id').Value := AItem.Id;
      AQuery.ExecSQL;
      qryPedidos_Produtos.Refresh;
   finally
      FreeAndNil(AQuery);
   end;
end;
...
function TModelEntidadePedido.GetPedidoNumero: Integer;
begin
   Result := -1;
   if not qryPedidos.IsEmpty then
      Result := qryPedidos.FieldByName('numero').AsInteger;
end;
...
procedure TModelEntidadePedido.InserePedido(APedido: TWKPedido);
var
   AQuery: TFDQuery;
   AIndex: Integer;
begin
   ...
   try
      ...
      if APedido.Numero <= 0 then
         begin
            ...
            AQuery.Open;
            APedido.Numero := AQuery.Fields[0].Value;
            qryPedidos.Refresh;
         end;
      for AIndex := 0 to APedido.ItensPedido.Tamanho - 1 do
         begin
            if APedido.ItensPedido.Items[AIndex].Id <= 0 then
               begin
                  with AQuery.SQL do
                     begin
                        Clear;
                        Add('INSERT INTO pedidos_produtos');
                        Add('   (numero_pedido, codigo_produto, quantidade, valor_unitario)');
                        Add('VALUES');
                        Add('   (:numero_pedido, :codigo_produto, :quantidade, :valor_unitario)');
                        Add('RETURNING id');
                     end;
                  // Definição dos parâmetros
                  AQuery.ParamByName('numero_pedido').DataType := ftInteger;
                  AQuery.ParamByName('numero_pedido').ParamType := ptInput;
                  AQuery.ParamByName('numero_pedido').Value := APedido.Numero;
                  AQuery.ParamByName('codigo_produto').DataType := ftString;
                  AQuery.ParamByName('codigo_produto').ParamType := ptInput;
                  AQuery.ParamByName('codigo_produto').Value := APedido.ItensPedido.Items[AIndex].CodigoProduto;
                  AQuery.ParamByName('quantidade').DataType := ftFloat;
                  AQuery.ParamByName('quantidade').ParamType := ptInput;
                  AQuery.ParamByName('quantidade').Value := APedido.ItensPedido.Items[AIndex].Quantidade;
                  AQuery.ParamByName('valor_unitario').DataType := ftFloat;
                  AQuery.ParamByName('valor_unitario').ParamType := ptInput;
                  AQuery.ParamByName('valor_unitario').Value := APedido.ItensPedido.Items[AIndex].ValorUnitario;
                  AQuery.Open;
                  APedido.ItensPedido.Items[AIndex].Id := AQuery.Fields[0].Value;
               end;
         end;
      AQuery.Close;
   finally
...
procedure TModelEntidadePedido.qryPedidosAfterOpen(DataSet: TDataSet);
begin
   qryPedidos_Produtos.Open();
end;

procedure TModelEntidadePedido.qryPedidosBeforeClose(DataSet: TDataSet);
begin
   qryPedidos_Produtos.Close();
end;
